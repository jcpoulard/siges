...
public function searchFeesToExempt($student,$level,$acad) 
  {
  	...
  	$criteria->distinct='true';
			...
			//if(($student=='')&&($level==''))
			 $criteria->condition = ' (b.student='.$student.' AND b.balance >0 AND b.fee_totally_paid=0 AND b.academic_year='.$acad.') OR (f.checked=0 AND (f.id not in(select fee from scholarship_holder sh where student='.$student.' AND partner IS NULL AND academic_year='.$acad.' AND percentage_pay=100 )  ) )  AND level =1 AND academic_period='.$acad;
			   
	        $criteria->join = 'inner join fees_label fl on(fl.id=f.fee) inner join billings b on(b.fee_period=f.id)';
  			
			$criteria->select = ' f.id, fl.fee_label, f.amount,f.date_limit_payment';
