...
class HomeworkController extends Controller
{
	...
	$room = strtr( $room, $this->pa_daksan );
    $level = strtr( $level, $this->pa_daksan );
    $section = strtr( $section, $this->pa_daksan );
    $shift = strtr( $shift, $this->pa_daksan );
   
    $name_acad = strtr( $acad_name, $this->pa_daksan );
    
     $course_name = strtr( $course_name_, $this->pa_daksan );
...
$path=$homework.'/'.$name_acad.'/'.$shift.'/'.$section.'/'.$level.'/'.$room;					   
								 ...
								 $model->setAttribute('academic_year',$acad);
										   ...
										   if($model->save())
			                                 {  
			                                 	 ...
			                                 	 $this->redirect(array('/guest/homework/view/id/'.$model->id.'/from/stud')); 
	...
	public function actionUpdate()
	{
		...
		if(isset($_POST['Homework']))
		{
		 ...
		 if(($this->room_id!="")&&($this->course_id!=""))//&&($this->homework_id!=""))
	        {   
			          ...
			          //change DOCUMENT NAME in new path
		                       ...
		                       for ($i = 0; $i < count($explode_new_fileName); $i++)
		                         {  
		                         	 ...
		                         	 $new_target =  Yii::app()->basePath.'/../'.$old_path.'/'.$newname;
						             $old_target =  Yii::app()->basePath.'/../'.$old_path.'/'.$explode_new_fileName[$i];
											...
											if ($size > 4*1024*1024)
										      {
											 ...
											 else
											   {
													...
													$newname_1 = $course_name.'_'.date('Y-m-d').'_'.$rnd.'.'.$ext; 
													
													 if($final_new_name!='')
													   { 
													   	   $model->setAttribute('attachment_ref',$newname_1.'/'.$final_new_name);
													     }
													 else    
									        ...
									        else
	                                         {
												...
												$mesage=Yii::t('app','Use one of these extensions: "odt", "pdf", "doc", "ods", "fods", "fodt", "odb", "odi", "oos", "oot", "otf", "otg", "oth", "oti", "otp", "ots", "sdp", "sds", "sdv", "sfs", "ods", "smf", "sms", "std", "stw", "sxg", "sxm", "vor", "sda", "sdc", "sdd", "odf", "odg", "ott", "pub", "rtf", "sdw", "sxc", "sxw", "bau", "dump", "fodg", "fodp", "odm", "odp", "otc", "psw", "sdb", "sgl", "smd", "stc", "sti", "svm", "sxd", "uot".');
   ...
   public function getRoom($id)
	...
	public function getRoomByStudentId($id)
	{
        ...
        $result=$room->find(array('alias'=>'r',
                                     ...
                                     'join'=>'left join room_has_person rh on(rh.room = r.id)',
                                     'condition'=>'rh.students=:studID AND rh.academic_year=:acad',
