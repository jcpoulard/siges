     ...
     <div class="span3" style="margin-left:0px;" >                        
                        ...
                        <div class="left" >
								...
								<?php 
									 ...
									 echo '<input type="hidden" id="start_date_" name="start_date_" value="" />';
                                     echo '<input type="hidden" id="end_date_" name="end_date_" value="" />';
      
									?>
									
									<div class="col-submit">
											                
										<button class="fa fa-print btn btn-success"  id="" name="" onclick="printDiv('print_receipt');"> <?php echo Yii::t('app','Print'); ?></button> <?php 																	 
...
<?php     
 echo ' <div id="print_receipt" style="display: none">
           <table style="font-size:12px; background-color:#F4F6F6;">
		           <tr>
		               <td style="width:70px; "></td><td style=" "></td>';
		      ...
		      $room_array=$modelRoom->findAll(array('alias'=>'r',
		         ...
		         foreach($room_array as $room)
		           {
		           	   $column_number++;
		                 	echo '<td style="text-align:center;  background-color: #F1F1F1; border-left: 1px solid #FFFFFF;"><b>'.$room->short_room_name.'</b></td>';
		           	}
		                 	...
		                 	if($nb_line!=null)
		                 	   $nb_line++;
		                          	...
		                          	echo '<tr>';  
		                          	  ...
		                          	  if($pass==1)
		                          	    { echo '<td rowspan="'.$nb_line.'" style=" width:50px; text-align:center;  background-color: #F1F1F1; border-bottom: 1px solid #FFFFFF; "><b>'; 
		         echo getShortDay( getDayNumberByShortDay(Yii::t('app',$d->format('D'))) ).' <br/>('.ChangeDateFormat($date_).')';   
		                       echo '</b></td>';    	    
		                          	       $nb_line=1;
		                          	      $pass=2;
		                          	    }
				                          ...
				                          foreach($sch_times as $times )
		                                    {
		                                    	echo '<tr>';  
				                          	      ...
				                          	      echo '<td style="border-left: 1px solid #FFFFFF; border-bottom: 1px solid #C8CACE;">'.date("H:i",strtotime($times["start_time"])).' - '.date("H:i",strtotime($times["end_time"])).'</td>';
				                             
					                          if($room_array!=null)
				                                 {  foreach($room_array as $room)
					                                  {
					                               	     echo '<td  style=" text-align:center; border-left: 1px solid #FFFFFF; border-bottom: 1px solid #C8CACE;">';
				                                 ...
				                                 {
				                                 	echo '<td  style=" text-align:center; border-left: 1px solid #FFFFFF; border-bottom: 1px solid #C8CACE;">  </td>';
				                                 	} 
				                           ...
				                           {
				                              echo '<tr><td style="width:99px; border-left: 1px solid #FFFFFF; border-bottom: 1px solid #C8CACE;"> </td>';
				                               if($room_array!=null)
				                                 {  foreach($room_array as $room)
					                                  {
					                               	     echo '<td  style=" text-align:center; border-left: 1px solid #FFFFFF; border-bottom: 1px solid #C8CACE;"> ';
 ...
 $this->widget('ext.fullcalendar.FullCalendar', array(
    ...
    'htmlOptions'=>array(
       ...
       'options'=>array(
        ...
        //'firstDay' =>1, // returns the day number of the week, works!
        'defaultView'=>'agendaWeek',
        //'columnFormat' =>'dddd D',//'dddd D.M.',
        'timeFormat' =>'HH:mm',
        //'scrollTime'=>$shift_time_start,//'08:00:00',
       ...
       // 'slotWidth'=>,
        ...
        'viewRender'=> 'js:function(view) {
			     var sta_day=moment(view.start).format("YYYY-MM-DD");
			     var end_day=moment(view.end).format("YYYY-MM-DD");
			     document.getElementById("start_date_").value = sta_day;
			     document.getElementById("end_date_").value = end_day;
			     
			    //alert(\'Visible start date \' + view.title + \'Visible end \' + sta_day);
			}',
