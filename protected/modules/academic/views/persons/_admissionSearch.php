...
<?php $pageSize=Yii::app()->user->getState('pageSize',Yii::app()->params['defaultPageSize']); // set controller and model for that before
          ...
          $to_search1 ='';
          $to_search2 ='';
          $daprovider=$model->admissionSearch_fullname('null','null');
           ...
           if($this->f_name!='')
		  	...
		  	if(($to_search1!='')&&($to_search2==''))
		  	   { $to_search =$to_search1;
		  	       $daprovider = $model->admissionSearch($to_search);
		  	     }
		  	elseif(($to_search1=='')&&($to_search2!=''))
		  	    ...
		  	    elseif(($to_search1!='')&&($to_search2!=''))
		  	      {   
		  	          $daprovider = $model->admissionSearch_fullname($to_search1,$to_search2);
		  	      }


   if(($to_search!='')&&($daprovider->getData()==null)&&($this->data==1))
      $this->redirect(array("/academic/persons/admission",'n'=>$to_search2,'pn'=>$to_search1,'vi'=>9));
   else
     {  
       ...
       if($daprovider->getData()!=null)
     	 {
     	   if(($to_search!='')||(($to_search1!='')&&($to_search2!=''))) 
     	     {  echo '<div style="margin-top:30px; margin-bottom:-30px;">';
     	     	 echo CHtml::link(Yii::t('app','Fill Form'),Yii::app()->createUrl("/academic/persons/admission",array('n'=>$to_search2,'pn'=>$to_search1,'vi'=>9)), array('class'=>'btn btn-secondary'));//
     	     	 echo '</div>';
     	       }
		...
		$this->widget('zii.widgets.grid.CGridView', array(
					...
					'columns'=>array(
						...
						'name'=>'last_name',
						 'type' => 'raw',
		                 'value'=>'CHtml::link($data->last_name,Yii::app()->createUrl("/academic/persons/viewForReport",array("id"=>$data->id,"isstud"=>1,"from"=>"adm","pg"=>"adm","n"=>"'.$to_search2.'","pn"=>"'.$to_search1.'","vi"=>9)))',
		                                   
		                                    ),
		       array(
						'name'=>'first_name',
						 'type' => 'raw',
		                 'value'=>'CHtml::link($data->first_name,Yii::app()->createUrl("/academic/persons/viewForReport",array("id"=>$data->id,"isstud"=>1,"from"=>"adm","pg"=>"adm","n"=>"'.$to_search2.'","pn"=>"'.$to_search1.'","vi"=>9)))',
