...
$acad=Yii::app()->session['currentId_academic_year']; 
            ...
            if(isset($_POST['Mails']))
		{
               ...
               // Yii::app()->user->setFlash(Yii::t('app','NoEmail'), Yii::t('app',"$this->teacher_id"));
                //echo $this->teacher_id;
            ...
            if($model_teacher->email!=null && $model_teacher->email!=''){
                    if(isset($_POST['Mails']['group_email'])){
                        ...
                        if(isset($_POST['create'])){
                            ...
                            $string_sql = "SELECT p.id, p.email, p.last_name, p.first_name FROM persons p join room_has_person rh on (p.id = rh.students) where rh.room = $this->groupemail and rh.academic_year = $acad_sess and p.email is not null";    
                        ...
                        foreach($student_data as $sd){
                            if($sd->email!=''){
                                ...
                                $name='=?UTF-8?B?'.base64_encode($model_teacher->first_name.' '.$model_teacher->last_name.Yii::t('app',' From ').$school_name).'?=';
...
$acad=Yii::app()->session['currentId_academic_year']; 				  
			...
			if($model->save()) {
                ...
                // Send email to all students for specific rooms 
                    if($this->groupemail==3){
                        ...
                        $sql_room_st = "SELECT p.id, p.email, p.active, p.is_student, r.students, r.room, r.academic_year  FROM persons p inner join room_has_person r on (r.students = p.id) where r.room =".$this->roomemail." AND r.academic_year = ".$acad_sess."";
        ...
        public function actionMailbox(){
         ...
         if(isset($_POST['Mails'])){ // Verifie que POST['Mails']
            ...
            $name='=?UTF-8?B?'.base64_encode($sender_name.Yii::t('app',' From ').$school_name).'?=';
                ...
                Yii::app()->user->setFlash(Yii::t('app','SentMail'), Yii::t('app','Email send with success !'));
                $this->redirect(array("/academic/mails/mailbox/mn/std/from/stud/?loc=inb"));
	...
	public function actionCreate()
	{
		$model = new Mails;
                ...
                $email_sender = null;
                //$id_sender = Yii::app()->user->personid;
		...
		if(isset($_POST['Mails']))
		{
			$model->attributes=$_POST['Mails'];
                       // $model->sender = $email;
                        ...
                        if(isset($_GET['de']) && $_GET['de']=='ci'){
                           ...
                           // $model->receivers = $model_c->email;
                        }else{
                          $model->setAttribute('receivers', $model_p->email);   
                        //$model->receivers = $model_p->email;
                        }
			...
			if($model->save()) {
                                ...
                                // Prepare le mail : Le nom et l'entete 
                            $name='=?UTF-8?B?'.base64_encode($sender_fullname.Yii::t('app',' From ').$school_name_alt).'?=';
				$subject='=?UTF-8?B?'.base64_encode($model->subject).'?=';
