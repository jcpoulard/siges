...
class AjaxController extends Controller
{
    ...
    public function actionCity()
    {
       ...
       // $sql = 'SELECT id, `name` FROM city WHERE `name` LIKE "%' . $q . '%"';
		 $sql = 'SELECT id, amount, `fee_name` as name FROM fees WHERE `fee_name` LIKE "%' . $q . '%"';
    ...
    public function actionStudent()
    {
       ...
       // $sql = 'SELECT id, `name` FROM city WHERE `name` LIKE "%' . $q . '%"';
		 $sql = 'SELECT id, CONCAT(first_name," ",last_name) as name FROM persons WHERE `last_name` LIKE "%' . $q . '%" AND is_student = 1';
        $rows = Yii::app()->db->createCommand($sql)->queryAll();
        if ($rows)
            echo CJSON::encode($rows);
    }
